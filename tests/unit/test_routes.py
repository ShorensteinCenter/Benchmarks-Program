...
def test_index(client, mocker):
    ...
    mocker.patch('app.routes.pd.read_sql', return_value=(
        pd.DataFrame({
            'creation_timestamp': [datetime(year=2018, month=1, day=1), None],
            'subscribers': [12967, 76921],
            'open_rate': [0.2678, 0.8762]})
        )
    )
    ...
    assert '76921'.encode() not in response.data
...
def test_org_info_bad_session(client, params):
    ...
    with client.session_transaction() as sess:
        for k, v in params.items():
            sess[k] = v
    assert client.get('/org-info').status_code == 403
...
def test_get_list_data_bad_session(client, sess_value):
    ...
    if sess_value:
        with client.session_transaction() as sess:
            sess[sess_value] = 'foo'
    assert client.get('/get-list-data').status_code == 403
...
def test_get_list_data(client, mocker):
    ...
    response = client.get('/get-list-data')
...
def test_analyze_list(client, mocker, fake_list_data):
    ...
    response = client.post('/analyze-list')
